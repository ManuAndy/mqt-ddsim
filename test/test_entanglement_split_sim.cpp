...
TEST(EntanglementSplitSimulatorDDSIM, grover7) {
    ...
    std::map<std::string, std::size_t> resultAmp;
    for (size_t i = 0; i < NumberOfTimeMeasurements; ++i) {
        qc = std::make_unique<qc::QuantumComputation>("circuits/grover_7.qasm");
...
using namespace qc::literals;
TEST(EntanglementSplitSimulatorDDSIM, grover19) {
    auto qc = std::make_unique<qc::QuantumComputation>("circuits/grover_19.qasm");
    ...
    std::map<std::string, std::size_t> resultAmp;
    for (size_t i = 0; i < NumberOfTimeMeasurements; ++i) {
        qc = std::make_unique<qc::QuantumComputation>("circuits/grover_19.qasm");
...
TEST(EntanglementSplitSimulatorDDSIM, grover19_simple) {
    auto qc = std::make_unique<qc::QuantumComputation>("circuits/grover_19.qasm");
